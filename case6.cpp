...
void read_chip_top(vector<block_data> & block_placing,vector<region_area>& region_placing,float die_area[2][2],int&unit,int &block_num,int& region_num){
    //open chip_top.def
    ifstream file;
    file.open("case06/chip_top.def");//之後改成不要寫死===================================!!!!!!!!!!!!
     if (!file.is_open()) {
...
void read_blk_area(const int num_of_block,vector<block_data> & block_placing){
    ifstream file;
    string blk_type=block_placing[num_of_block].blk_type;
    string file_place="case06/"+blk_type+".def";
...
void read_cfg(vector<block_data> & block_placing){
    ifstream file;
    string file_place="case06_cfg.json";
    file.open(file_place);
     if (!file.is_open()) {
        cout<<"no such file exist";
    }
...
void parse_connection_matrix(vector<NETS> &connection_matrix ){
    ifstream file;
    string file_place="case06.json";
    file.open(file_place);
     if (!file.is_open()) {
        cout<<"no such file exist";
    }
...
int main(){
    ...
    read_chip_top(block_placing,region_placing,die_area,unit,block_num, region_num);//access into the chip_top file, get the data of each block and the placement of region
    //cout<<block_num<<endl;
    for(int i=0;i<block_num;i++){ 
        //cout<<i<<" ";
        read_blk_area(i,block_placing);//access into the file of various types of blocks, get the data of area
    }
    read_cfg(block_placing);

    
    parse_connection_matrix(connection_matrix);
    ...
    for(int i=0;i<block_placing.size();i++){
        ...
        if(!block_placing[i].is_feedthroughable){
            ...
            for(int j=0;j<block_placing[i].vertex.size();j++){
                for(int k=0;k<2;k++){
                    cout<<block_placing[i].vertex[j][k]<<" ";
                }
            }
        cout<<endl;
        }
 ...
 /*//below are used to check data
    //display segment start===================================
    cout<<unit<<endl;
    cout<<die_area[0][0]<<" "<<die_area[0][1]<<endl<<die_area[1][0]<<" "<<die_area[1][1]<<endl;
    for(int i=0;i<block_placing.size();i++){
        //cout<<block_placing[i].blk_name<<" "<<block_placing[i].blk_type<<" "<<block_placing[i].place[0]<<" "<<block_placing[i].place[1]<<" "<<block_placing[i].direction<<" ";
        cout<<block_placing[i].place[0]<<" "<<block_placing[i].place[1]<<" "<<block_placing[i].direction<<" ";
        for(int j=0;j<block_placing[i].vertex.size();j++){
            for(int k=0;k<2;k++){
                cout<<block_placing[i].vertex[j][k]<<" ";
        ...
        //cout<<"tbnn: "<<block_placing[i].through_block_net_num<<" ";// print cfg content:through_block_net_num
        /*cout<<"tbenn: ";
        for(int j=0;j<block_placing[i].through_block_edge_net_num_lst.size();j++){// print cfg content:through_block_edge_net_num
            for(int k=0;k<2;k++){
                for(int l=0;l<2;l++){
                    cout<<block_placing[i].through_block_edge_net_num_lst[j].start_and_end[k][l]<<" ";
                }
            }
            cout<<block_placing[i].through_block_edge_net_num_lst[j].number<<" ";
            
        }
        cout<<"bpr: ";
        for(int j=0;j<block_placing[i].block_port_region.size();j++){
            cout<<block_placing[i].block_port_region[j][0]<<" "<<block_placing[i].block_port_region[j][1]<<" ";
        }
        cout<<"if: ";
        if(block_placing[i].is_feedthroughable==1)cout<<"True"<<" ";
        else cout<<"False"<<" ";
        /*cout<<"it: ";
        if(block_placing[i].is_tile==1)cout<<"True"<<" ";
        else cout<<"False"<<" ";
        if(block_placing[i].is_feedthroughable==1)cout<<"True"<<" ";
        else cout<<"False"<<" ";
        cout<<endl;
    }*/
    /*cout<<region_placing.size();
    for(int i=0;i<region_placing.size();i++){
        cout<<region_placing[i].reg_name<<" ";
        for(int j=0;j<2;j++){
            cout<<region_placing[i].left_down[j]<<" ";
        }
        for(int j=0;j<2;j++){
            cout<<region_placing[i].right_up[j]<<" ";
        }
        cout<<endl;
    }
    cout<<block_num<<" "<<region_num;
    //display segment end ===================================*/
    /*cout<<"block port regions"<<endl;
    for(int i=0;i<block_placing.size();i++){
        for(int j=0;j<block_placing[i].block_port_region.size();j++){
            for(int k=0;k<2;k++){
                cout<<block_placing[i].block_port_region[j][k]*2000+block_placing[i].place[k]<<",";
            }
        }
        if(block_placing[i].block_port_region.size()!=0)cout<<endl;

            cout<<connection_matrix.size()<<endl;
    for(int i=0;i<connection_matrix.size();i++){
        cout<<"ID:"<<connection_matrix[i].ID<<",TX: "<<connection_matrix[i].TX<<",";
        cout<<"RX:";
        for(int j=0;j<connection_matrix[i].RX.size();j++)cout<<connection_matrix[i].RX[j]<<" ";
        cout<<endl;
        cout<<"NUM:"<<connection_matrix[i].NUM<<" ";
        cout<<endl<<"MUST_THROUGH:";
        for(int j=0;j<connection_matrix[i].MUST_THROUGH.size();j++){
            cout<<connection_matrix[i].MUST_THROUGH[j].BLK_name<<" ";
            for(int k=0;k<connection_matrix[i].MUST_THROUGH[j].ptr_data.size();k++)cout<<connection_matrix[i].MUST_THROUGH[j].ptr_data[k]<<" ";
        }
        cout<<endl<<"HMFT_MUST_THROUGH:";
        for(int j=0;j<connection_matrix[i].HMFT_MUST_THROUGH.size();j++){
            cout<<connection_matrix[i].HMFT_MUST_THROUGH[j].BLK_name<<" ";
            for(int k=0;k<connection_matrix[i].HMFT_MUST_THROUGH[j].ptr_data.size();k++)cout<<connection_matrix[i].HMFT_MUST_THROUGH[j].ptr_data[k]<<" ";
        }
        cout<<endl<<"TX_COORD: "<<connection_matrix[i].TX_COORD[0]<<" "<<connection_matrix[i].TX_COORD[1];
        cout<<endl<<"RX_COORD: ";
        for(int j=0;j<connection_matrix[i].RX_COORD.size();j++){
            cout<<connection_matrix[i].RX_COORD[j][0]<<" "<<connection_matrix[i].RX_COORD[j][1]<<",";
        }
        cout<<endl;
    }
    }*/